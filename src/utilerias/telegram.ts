...
export class Telegram {
  ...
  // private readonly modulo = AbstractConfiguration.API_NOMBRE || 'Backend';
  private readonly bot: TelegramBot;
  // public static tokenDefault = '5137845409:AAFlhqU7RzV02OnTu1Yj-n3XhL18Q6ZBfkQ';
  // private static idGrupoDefault = AbstractConfiguration.ID_TELEGRAM || '-969215861';
  // private readonly tokenTelegram: string;

  private get modulo() {
    return AbstractConfiguration.API_NOMBRE || "Backend";
  }

  public static get tokenDefault(): string {
    return "5137845409:AAFlhqU7RzV02OnTu1Yj-n3XhL18Q6ZBfkQ";
  }

  private static get idGrupoDefault(): string {
    return AbstractConfiguration.ID_TELEGRAM || "-969215861";
  }
  ...
  static getInstance(token?: string): Telegram {
    const tokenD = token || Telegram.tokenDefault;
  ...
  public enviarNotificacion = async (
    ...
    try {
      ...
      LoggerS3.getInstance()
        .getLogger()
        .info(`Error al enviar notificación ${error?.message}`);
    }
  };
  ...
  public enviarArchivo = async (
    parametros: {
      mensaje: string;
      archivo: { base64: string; nombre: string };
    },
    id?: string
  ): Promise<{ hayError: boolean }> => {
    const { archivo, mensaje } = parametros;
    try {
      await this.bot.sendMessage(id || Telegram.idGrupoDefault, mensaje, {
        parse_mode: "Markdown",
      });
  ...
  public enviarMensajeAUsuario = async (
    userId: string,
    mensaje: string,
    token?: string
  ): Promise<void> => {
    try {
      ...
      const botInstance = token
        ? Telegram.getInstance(token)
        : Array.from(Telegram.instances.values())[0];

      if (!botInstance) {
        throw new Error("No hay instancias de bots disponibles.");
      }

      await botInstance.sendMessageAsync(userId, mensaje, {
        parse_mode: "Markdown",
      });
...
export const BotsTelegram = () => {
  let catTokensBots = [Telegram.tokenDefault];
