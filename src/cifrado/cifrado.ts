...
export class Cifrado {
    ...
    private encriptarRSAPSK1 = (valor: string, keyPublica: string): ICifradoResponse => {
        ...
        try {
            const publica = ['-----BEGIN PUBLIC KEY-----', keyPublica, '-----END PUBLIC KEY-----'].join('\n');
    ...
    public descifraRSAPSK1 = (valorCifrado: string, keyPrivada: string): ICifradoResponse => {
        const resp: ICifradoResponse = {
            error: true,
            mensaje: '',
            valor: '',
        ...
        try {
            const llavePriv = ['-----BEGIN PRIVATE KEY-----', keyPrivada, '-----END PRIVATE KEY-----'].join('\n');
    ...
    private encriptarRSAOAEP = (valor: string, keyPublica: string): ICifradoResponse => {
        let valorCifrado = '';
        const resp: ICifradoResponse = {
            error: true,
            mensaje: '',
    ...
    public descifrarRSAOAEP = (valorCifrado: string, keyPrivada: string): ICifradoResponse => {
        const resp: ICifradoResponse = {
            error: true,
            mensaje: '',
            valor: '',
        ...
        try {
            const privada = ['-----BEGIN PRIVATE KEY-----', keyPrivada, '-----END PRIVATE KEY-----'].join('\n');
    ...
    private validaRSAPSK = (cadena: string, keyPublica: string, depth = 0): string => {
    ...
    private validaRSAOAEP = (cadena: string, keyPublica: string, depth = 0): string => {
    ...
    public validaCadenaRSA = (cadena: string, keyPublica: string, oaep = false): string => {
        switch (oaep) {
            case true: {
    ...
    private encriptar_AES_CBC_PKCS5 = (
        ...
        try {
            ...
            const hmac = crypto.createHmac('sha256', hmacKey).update(iv_cipherText).digest('base64');
            ...
            const iv_cipherText_hmac_base64 = Buffer.from(iv_cipherText_hmac).toString('base64');
            resp.error = false;
    ...
    private valida_AES_CBC_PKCS5 = (
        ...
        const {valor, error} = this.encriptar_AES_CBC_PKCS5(cadena, accesoSimetrico, codigoAutentificacionHash);
        if (error) {
            if (depth < MAX_RECURSION_DEPTH) {
                return this.valida_AES_CBC_PKCS5(cadena, accesoSimetrico, codigoAutentificacionHash, depth + 1);
            } else {
                return '';
            }
        }
    ...
    private valida_AES_GCM_NoPadding = (
        ...
        const {valor, error} = this.encriptar_AES_GCM_NoPadding(cadena, accesoSimetrico, codigoAutentificacionHash);
        if (error) {
            if (depth < MAX_RECURSION_DEPTH) {
                return this.valida_AES_GCM_NoPadding(cadena, accesoSimetrico, codigoAutentificacionHash, depth + 1);
            } else {
                return '';
            }
        }
    ...
    public descifrar_AES_CBC_PKCS5 = (
        ...
        try {
            ...
            const receivedHMAC = iv_cipherText_hmac.slice(cipherTextLength, iv_cipherText_hmac.length);
            const calculatedHMAC = crypto.createHmac('sha256', hmacKey).update(iv_cipherText).digest('base64');
    ...
    public validaCadenaAES = (
        ...
        if (gcm_noPadding) {
            return this.valida_AES_GCM_NoPadding(cadena, accesoSimetrico, codigoAutentificacionHash);
        } else {
            return this.valida_AES_CBC_PKCS5(cadena, accesoSimetrico, codigoAutentificacionHash);
        }
    };
}
